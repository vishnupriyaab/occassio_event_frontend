 ...
 ngOnInit(): void {
    ...
    this._photoService.getPhotos().subscribe({
      next: (response) => {
        ...
        if (response.success) {
          ...
          this.errorMessage = response.message || 'Failed to fetch Photos & videos features';
  ...
  handleSaveData(data: IPhotoCreate) {
    if (this.editingItem && this.editingItem._id) {
      this._photoService
        .updatePhoto(this.editingItem._id, data)
        .subscribe({
          next: (response) => {
          ...
          error: (error) => {
            this.errorMessage = 'An error occurred while updating Photo & Video Features';
      ...
      this._photoService.addPhoto(data).subscribe({
        next: (response) => {
          if (response.success) {
            ...
            this.errorMessage = response.message || 'Failed to add Photo & Video Features';
          }
        },
        error: (error) => {
  ...
  deletePhoto(item: IPhotoCreate) {
    if (confirm('Are you sure you want to delete this decoration?')) {
      this._photoService.deletePhoto(item._id).subscribe({
        ...
        error: (error) => {
          this.errorMessage = 'An error occurred while deleting Photo & Video Features';
  ...
  handleStatusChange(item: IPhotoCreate, newStatus: boolean) {
    this._photoService
      .togglePhotoStatus(item._id, newStatus)
      .subscribe({
        next: (response) => {
          if (response.success) {
